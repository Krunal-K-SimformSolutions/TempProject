 ...
 * request.
 * @property {Record<string, any>} setting - This is the setting object that is passed to the action.
 */
 ...
 */
export const authorizedAPI: ApisauceInstance = apiConfig(Config.API_URL ?? '');
export const unauthorizedAPI: ApisauceInstance = apiConfig(Config.API_URL ?? '');
...
function apiWithCancelToken<Response>(
  { api, method, url, params, data, setting }: ApiConfig,
...
export function createAsyncThunkWithCancelToken<Response extends ResponseBound>(
  ...
  return createAsyncThunk<Response, ThunkArg, ThunkApiConfig>(
    typePrefix,
    async ({ data, params, setting }, thunkApi) => {
      ...
      try {
        const response = await apiWithCancelToken<Response>(
          { api, method, url, data, params, setting },
          source
        );
        if (response.ok) {
